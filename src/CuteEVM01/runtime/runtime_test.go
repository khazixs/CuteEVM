...
func BenchmarkCall(b *testing.B) {
	...
	abi2, err := abi.JSON(strings.NewReader(definition))
	if err != nil {
		b.Fatal(err)
	}

	cpurchase, err := abi2.Pack("confirmPurchase")
	if err != nil {
		b.Fatal(err)
	}
	creceived, err := abi2.Pack("confirmReceived")
	if err != nil {
		b.Fatal(err)
	}
	refund, err := abi2.Pack("refund")
	if err != nil {
		b.Fatal(err)
	}
